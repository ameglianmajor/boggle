# This class is used to initialize a Boggle board, load words from a
# dictionary, and find all the valid words on the Boggle board that
# is initialized.
class BoggleSolver
  ...
  # For a fixed board size, this algorithm is typically O(N), where N is the
  # number of words in the dictionary. This assumes that the strings being
  # searched for are words in the English language. Since the longest word is
  # 32 letters, the word length can be treated as a constant from a
  # computational complexity perspective. Branching of visit_chains also needs
  # to be considered for worst case analysis, but branching is not a
  ...
  def valid_word?(word)
    atomic_elements = self.class.word_array word
    found_word_chains = atomic_elements[1..-1]
      .inject(start_coordinates atomic_elements[0]) do |visit_chains, atomic_element|
        ...
        visit_chains.map! do |visit_chain|
          augmented_visit_chains(visit_chain, atomic_element)
        end
  ...
  def augmented_visit_chains(visit_chain, next_atomic_string)
    ...
    matching_visits =
      possible_visits.select { |x| @board[x[0]][x[1]] == next_atomic_string }
  ...
  def one_dim_neighbors(n)
    [n - 1, n, n + 1].select { |x| 0 <= x && x < @length }
  end

  def load_words(word_list_filepath)
    File.open("#{ word_list_filepath }", 'r') do |file_handle|
